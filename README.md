...
**核心流程：热键录音 → 本地 ASR 转写 → 个人词库注入 → LLM 智能纠错+润色 → 剪贴板输出 → LLM后台自动强化词库**
...
- **个人词库系统**：LLM 智能纠错 + LLM自动术语发现 + 分层注入转写LLM + 黑名单 + 旅客模式
...
- **注意：**请开始使用前自行设定热键，实际 clone 项目的默认热键可能不为上表所设计。
