...
class VentiVoiceApp:
    def __init__(self):
        ...
        self.corpus = Corpus(str(corpus_path))
    ...
    def _try_load_llm(self):
        try:
            self._llm = LLMProcessor(self.config, self._current_llm_provider)
        except Exception:
            self._llm = None
    ...
    def _switch_llm_provider(self, provider_name: str):
        ...
        try:
            self._llm = LLMProcessor(self.config, provider_name)
    ...
    def _process_audio(self, audio):
        try:
            ...
            raw_text = self.corpus.apply_corrections(raw_text)
    ...
    def _build_ui(self):
        ...
        # 档位选择
        tier_frame = ttk.Frame(main, style="Dark.TFrame")
        tier_frame.pack(fill=tk.X, pady=(0, 6))

        ttk.Label(tier_frame, text="档位:", style="Dark.TLabel").pack(side=tk.LEFT)
        self._tier_var = tk.IntVar(value=self._current_tier)
        for t, label in [(1, "Clean"), (2, "Refine"), (3, "Rewrite")]:
            ttk.Radiobutton(tier_frame, text=f"{t}. {label}", variable=self._tier_var,
                           value=t, style="Dark.TRadiobutton",
                           command=lambda t=t: self._set_tier(t)).pack(side=tk.LEFT, padx=(8, 0))
        ...
        ttk.Button(btn_frame, text="清空", command=self._clear_result).pack(side=tk.RIGHT)

        # 纠错面板
        correct_frame = ttk.LabelFrame(main, text="纠错", style="Dark.TLabelframe", padding=4)
        correct_frame.pack(fill=tk.X, pady=(0, 6))

        row1 = ttk.Frame(correct_frame, style="Dark.TFrame")
        ...
        row2 = ttk.Frame(correct_frame, style="Dark.TFrame")
        ...
        row3 = ttk.Frame(correct_frame, style="Dark.TFrame")
        row3.pack(fill=tk.X, pady=2)
        ttk.Button(row3, text="提交纠错", command=self._submit_correction).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Button(row3, text="取消", command=self._clear_correction).pack(side=tk.LEFT)
        ...
        # 热键设置面板 (在状态栏上方)
        hotkey_frame = ttk.LabelFrame(main, text="热键设置", style="Dark.TLabelframe", padding=4)
        hotkey_frame.pack(fill=tk.X, pady=(0, 6), side=tk.BOTTOM)

        # 模式切换行
        mode_row = ttk.Frame(hotkey_frame, style="Dark.TFrame")
        ...
        for tier in (1, 2, 3):
            row = ttk.Frame(hotkey_frame, style="Dark.TFrame")
    ...
    def _submit_correction(self):
        ...
        if wrong and correct:
            ...
            self._result_text.delete("1.0", tk.END)
            self._result_text.insert("1.0", current.strip())
